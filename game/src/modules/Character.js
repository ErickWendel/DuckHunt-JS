...
class Character extends AnimatedSprite {
   ...
   * @param {String} spritesheet The key used by PIXI Assets to retrieve the loaded spritesheet
   * @param {{name:String, animationSpeed:Number}[]} states The states defined in the spritesheet
   */
  constructor({ spriteId, textures, states }) {
    for (const textureKey of Object.keys(textures)) {
      ...
      parts.length -= 1; // remove media file part
    ...
    this.animationSpeed = states[0].animationSpeed;
